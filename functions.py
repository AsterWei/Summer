...
class Dataset_3DCNN(data.Dataset):
    ...
    def read_images(self, path, selected_folder, use_transform):
        ...
        for i in self.frames:
            image = Image.open(os.path.join(path, selected_folder, 'frame_{:04d}.png'.format(i))).convert('L')
...
class Dataset_CRNN(data.Dataset):
    ...
    def read_images(self, path, selected_folder, use_transform):
        X = []
        for i in self.frames:
            image = Image.open(os.path.join(path, selected_folder, 'frame_{:04d}.png'.format(i)))
